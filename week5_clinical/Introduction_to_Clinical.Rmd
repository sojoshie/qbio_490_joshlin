...
```{r}
write.csv(clinic, "/home1/jlin7423/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/jlin7423/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
```{r Exercise 4.1}
# create plot here
plot('age_at_diagnosis', 'lymph_nodes_examined_count', xlim = c(0, 100), ylim = c(0, 600))
```
...
```{r Exercise 4.2}
# create plot here
boxplot(age_at_diagnosis~gender, data = clinic)
```
...
# 2. What does the x axis represent? Time elapsed since diagnosis in days

# 3. What age group appears to have the greatest rate of survival? Middle aged (between 35 to 50)

# 4. What age group appears to have the worst rate of survival? Young aged

# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients? 0.30 (30%)

# 6. What is the p-value? Is it statistically significant and what does that tell us about the relationship between age and breast cancer survival? p = 0.2, which is less than 0.05, meaning it is not statistically significant and that there is not sufficient evidence to show a relationship between age and breast cancer survival. 
